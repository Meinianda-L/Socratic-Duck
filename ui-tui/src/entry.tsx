...
if (!process.stdin.isTTY) {
  console.log('socratic-duck: no TTY')
  process.exit(0)
}
...
const dumpNotice = (snap: MemorySnapshot, dump: HeapDumpResult | null) =>
  `socratic-duck: ${snap.level} memory (${formatBytes(snap.heapUsed)}) — auto heap dump → ${dump?.heapPath ?? '(failed)'}\n`
...
onError: (scope, err) => {
  ...
  process.stderr.write(`socratic-duck lifecycle ${scope}: ${message.slice(0, 2000)}\n`)
},
onSignal: signal => {
  resetTerminalModes()
  process.stderr.write(`socratic-duck lifecycle: received ${signal}\n`)
}
})
...
onCritical: (snap, dump) => {
  resetTerminalModes()
  process.stderr.write(`socratic-duck lifecycle: memory critical exit heap=${formatBytes(snap.heapUsed)} rss=${formatBytes(snap.rss)}\n`)
  process.stderr.write(dumpNotice(snap, dump))
  process.stderr.write('socratic-duck: exiting to avoid OOM; restart to recover\n')
  process.exit(137)
},
